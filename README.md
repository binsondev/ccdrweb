...
- App (hostname on port 80): `http://coreextract.copass.in/`
- API (hostname on port 80): `http://coreextract-api.copass.in/`
- App (direct port): `http://<public-ip>:8081/`
- API (direct): `http://<public-ip>:7421/`

These hostnames are `server_name`s on the existing FleetStalk nginx (same public IP as `fleetstalk.com`). Point DNS:

- `coreextract.copass.in` A → `103.177.26.199`
- `coreextract-api.copass.in` A → `103.177.26.199`
